...
class RequestManager {
    ...
    async __internal_startup() {
                    ...
                    switch (req.method.toUpperCase()) {
                        case "GET":
                            ...
                            this.instance.log.info(
                                "response",
                                `${chalk.bgBlack(chalk.green(" GET "))} ${chalk.bgBlack(chalk.red(` ${res.statusCode} `))} ${chalk.bold(
                                    req.url,
                                )} ${
                                    this.instance.configurationManager.config.logQueryParameters
                                        ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                                        : ""
                                }`,
                            );
                            break;
                        case "POST":
                            this.instance.log.info(
                                "response",
                                `${chalk.bgBlack(chalk.blue(" POS "))} ${chalk.bgBlack(chalk.red(` ${res.statusCode} `))} ${chalk.bold(
                                    req.url,
                                )} ${
                                    this.instance.configurationManager.config.logQueryParameters
                                        ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                        ...
                        case "DELETE":
                            this.instance.log.info(
                                "response",
                                `${chalk.bgBlack(chalk.red(" DEL "))} ${chalk.bgBlack(chalk.red(` ${res.statusCode} `))} ${chalk.bold(
                                    req.url,
                                )} ${
                                    this.instance.configurationManager.config.logQueryParameters
                                        ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                        ...
                        case "OPTIONS":
                            if (this.instance.configurationManager.config.logOptionsRequests) {
                                this.instance.log.info(
                                    "response",
                                    `${chalk.bgBlack(chalk.cyan(" OPT "))} ${chalk.bgBlack(chalk.red(` ${res.statusCode} `))} ${chalk.bold(
                                        req.url,
                                    )} ${
                                        this.instance.configurationManager.config.logQueryParameters
                                            ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                        ...
                        case "PROPFIND":
                            this.instance.log.info(
                                "response",
                                `${chalk.bgBlack(chalk.cyan(" PFI "))} ${chalk.bgBlack(chalk.red(` ${res.statusCode} `))} ${chalk.bold(
                                    req.url,
                                )} ${
                                    this.instance.configurationManager.config.logQueryParameters
                                        ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                        ...
                        case "PROPPATCH":
                            this.instance.log.info(
                                "response",
                                `${chalk.bgCyan(chalk.cyan(" PPA "))} ${chalk.bgBlack(chalk.red(` ${res.statusCode} `))} ${chalk.bgBlack(
                ...
                if (res.statusCode.toString()[0] === "5") {
                    switch (req.method.toUpperCase()) {
                        ...
                        case "PROPPATCH":
                            this.instance.log.info(
                                "response",
                                `${chalk.bgCyan(chalk.cyan(" PPA "))} ${chalk.bgBlack(chalk.red(` ${res.statusCode} `))} ${chalk.bold(
                                    req.url,
                                )} ${
                                    this.instance.configurationManager.config.logQueryParameters
                                        ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
        ...
        this.app.use(async (req, res, next) => {
            let queryResult = await this.instance
                .database`INSERT INTO public.request_manager_log (request_timestamp, request_method, request_path, request_body)
            ...
            switch (req.method.toUpperCase()) {
                case "GET":
                    ...
                    this.instance.log.info(
                        "request",
                        `${chalk.bgBlack(chalk.green(" GET "))} ${chalk.bold(req.url)} ${
                            this.instance.configurationManager.config.logQueryParameters
                                ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                                : ""
                        }`,
                ...
                case "POST":
                    this.instance.log.info(
                        "request",
                        `${chalk.bgBlack(chalk.blue(" POS "))} ${chalk.bold(req.url)} ${
                            this.instance.configurationManager.config.logQueryParameters
                                ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                                : ""
                        }`,
                ...
                case "DELETE":
                    this.instance.log.info(
                        "request",
                        `${chalk.bgBlack(chalk.red(" DEL "))} ${chalk.bold(req.url)} ${
                            this.instance.configurationManager.config.logQueryParameters
                                ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                                : ""
                        }`,
                ...
                case "OPTIONS":
                    if (this.instance.configurationManager.config.logOptionsRequests) {
                        this.instance.log.info(
                            "request",
                            `${chalk.bgBlack(chalk.cyan(" OPT "))} ${chalk.bold(req.url)} ${
                                this.instance.configurationManager.config.logQueryParameters
                                    ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                                    : ""
                            }`,
                ...
                case "PROPFIND":
                    this.instance.log.info(
                        "request",
                        `${chalk.bgBlack(chalk.cyan(" PFI "))} ${chalk.bold(req.url)} ${
                            this.instance.configurationManager.config.logQueryParameters
                                ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                                : ""
                        }`,
                ...
                case "PROPPATCH":
                    this.instance.log.info(
                        "request",
                        `${chalk.bgCyan(chalk.cyan(" PPA "))} ${chalk.bold(req.url)} ${
                            this.instance.configurationManager.config.logQueryParameters
                                ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                                : ""
                        }`,
                ...
                default:
                    this.instance.log.error(
                        "core_requests",
                        `ERROR IN REQUEST LOGGER, UNKNOWN REQUEST TYPE: ${req.method}, ${chalk.bold(req.url)} ${
                            this.instance.configurationManager.config.logQueryParameters
                                ? JSON.stringify(req.query) !== "{}" && JSON.stringify(req.query)
                                : ""
                        }`,
        ...
        this.app.get("/core/test/self-ping", async (req, res) => {
            return res.json({ success: true });
        });

        this.instance.authorization.publicPaths.push("/login/instance/metadata");
        this.app.get("/login/instance/metadata", (req, res) => {
            return res.json({
        ...
        this.app.get("/login/user/:username", async (req, res) => {
        ...
        this.app.get("/login/user/:username/avatar", async (req, res) => {
            ...
            if (await user.doesExist()) {
                res.status(200);
                return res.sendFile(path.join(this.instance.filesystem.commonPaths.UserSystemDirectory(user.username), "avatar128.webp"));
            } else {
                return res.status(404);
            }
        });

        this.app.post("/login/user/authenticate", async (req, res) => {
        ...
        this.app.post("/login/user/logout", async (req, res) => {
        ...
        this.app.get("/login/instance/background", async (req, res) => {
            res.status(200);
            return res.sendFile(path.join(this.instance.filesystem.commonPaths.SystemDirectory(), "loginBackground.avif"));
        });

        this.app.get("/core/login/notice", async (req, res) => {
            return res.json({
                author: "System",
        ...
        this.app.get("/panel/logo/small", async (req, res) => {
            return res.sendFile(path.join(this.instance.filesystem.commonPaths.SystemDirectory(), "instanceLogo32.webp"));
        });

        this.app.get("/panel/logo/medium", async (req, res) => {
            return res.sendFile(path.join(this.instance.filesystem.commonPaths.SystemDirectory(), "instanceLogo40.webp"));
        });

        this.app.get("/panel/logo/large", async (req, res) => {
            return res.sendFile(path.join(this.instance.filesystem.commonPaths.SystemDirectory(), "instanceLogo128.webp"));
        });

        this.app.get("/core/panel/applications", async (req, res) => {
        ...
        this.app.get("/core/panel/applications/app/largeGrid/:applicationId", async (req, res) => {
            ...
            const app = this.instance.applications.loadedApplications.find((a) => a._applicationParameters.id === applicationId);
            ...
            await fs.mkdir(
                path.join(this.instance.filesystem.commonPaths.GlobalCacheDirectory(), "panel/applications", app._applicationParameters.id),
                {
                    recursive: true,
                },
            );

            if (!(await this.instance.filesystem.doesPathExist(path.join(app?._applicationSourcePath, "./assets/icon.png")))) {
                return res.sendFile(path.join(this.instance.filesystem.commonPaths.GlobalCacheDirectory(), "panel", "invalidIcon.webp"));
            }
        ...
        this.app.get("/core/panel/quick-shortcuts", async (req, res) => {
            ...
            const pinnedApplications: YourDashApplication[] = query[0].pinned_applications
                .map((a: string) => this.instance.applications.loadedApplications.find((i) => i._applicationParameters.id === a))
            ...
            return pinnedApplications.map((app) => {
                ...
                return res.json({
                    displayName: app._applicationParameters.displayName,
                    id: app._applicationParameters.id,
                    url: app._applicationParameters.externalFrontend?.url || "example.com",
                });
            });
        });
        ...
        this.app.post("/core/panel/quick-shortcuts/create", async (req, res) => {
            const applicationId = (req.body as { id: string }).id;
            const username = this.getRequestUsername();

            if (this.instance.applications.loadedApplications.find((i) => i._applicationParameters.id === applicationId)) {
                try {
                    const previousPins = await this.instance.database`SELECT pinned_applications
                                                                          FROM public.panel_configuration
                                                                          WHERE username = ${username};`;

                    if (previousPins[0].pinned_applications.includes(applicationId)) {
                        return { success: false };
                    }

                    const newPins = [...previousPins[0].pinned_applications, applicationId];
        ...
        this.app.get("/core/panel", async (req, res) => {
            const username = this.getRequestUsername();
        ...
        this.app.use(
            ...
            (error, req, res, next) => {
                if (!error.cause) {
                    ...
                    return res.json({
                        status: 500,
                        message: "Internal server error, if you are a developer or administrator, please check the console.",
                    });
                }
