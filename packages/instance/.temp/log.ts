...
export default class Log {
    ...
    constructor(instance: Instance) {
        ...
        // TODO: make hot reloading work with this
        // @ts-ignore
        ...
        // };
        // // @ts-ignore (TODO: Maybe relook at this)
    ...
    _internal_writePrompt() {
        if (!this.instance?.configurationManager?.hasFeature(YourDashFeatureFlags.SlashCommands)) return this;
                ...
                this._internal_cursorTo(0, this._internal_getWindowSize()[1] - 3, () => {
                    ...
                    process.stdout.write("-".repeat(this._internal_getWindowSize()[0]), () => {
                        ...
                        this._internal_cursorTo(0, this._internal_getWindowSize()[1] - 2, () => {
                            ...
                            process.stdout.write(
                                `   YourDash Pre-Alpha ${
                                    this.instance.configurationManager?.config.isDevMode ? `[${this.addEmphasisToString("DEV Mode")}] ` : ""
                                }`,
                                () => {
    ...
    rawLog(level: string, ...message: (string | Uint8Array)[]) {
        if (level.length === 0) {
        ...
        return this.log(LogType.RAW, level, ...message);
    }
    ...
    createBanner(bannerText: string): string {
        const LOG_WIDTH = this._internal_getWindowSize()[0] - (this.instance.log.metaLength + 6);
        return "-".repeat((LOG_WIDTH - bannerText.length) / 2) + bannerText + "-".repeat((LOG_WIDTH - bannerText.length) / 2);
    }
